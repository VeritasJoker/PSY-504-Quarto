---
title: "Lab: Intro to MLM Exercise/Walkthrough"
...
Let's consider a linear regression scenario with a toy dataset. Analysis question: How does practice influences the reading age of toy characters Dataset link: <https://uoepsy.github.io/data/toy2.csv> contains information on 129 different toy characters that come from a selection of different families/types of toy.
...
`r qbegin(qcounter())` Below is some code that fits a model of "reading age" (`R_AGE`) predicted by hours of practice (`hrs_week`). Line 2 then gets the 'fitted' values from the model and adds them as a new column to the dataset, called `pred_lm`. The fitted values are what the model predicts for every individual observation (every individual toy in our dataset).
...
```{r}
# The line is the same for every type of toy since we are fitting a general model for all toys. However, the line obviously does not fit some types of toys (both the intercept and slope.
```
...
# For the first two models, the green and blue lines have different intercepts for different toy types, but still have the same slope for all lines. For the third model, the orange lines have different intercepts and slopes, which fits some toy types, like Minecraft and Sock Puppets, better.
...
# The solutions are more detailed and also explains the reasons, which are pretty nice.
...
# A1 is the standard deviation of the distances from all the toy-type lines to the black line

# A2 is the standard deviation of the distances from all the individual datapoints (toys) to their respective toy-type lines

# A3 is where the black line cuts the y axis (at x=0)

# A4 is the slope of the black line
...
# 4.1
print(length(unique(efdat$PID)))

# 4.2
efdat %>% count(PID) %>% filter(n == 15)
print("None")

# 4.3
efdat %>% count(PID) %>% summary()
print("Mean is 12, min is 10")

# 4.4
efdat %>% count(PID, audio) %>% summary()
print("Yeah, each has at least 2")
```

`r qend()` `r solbegin(label="1 - read in the data ", slabel=F,show=T, toggle=params$TOGGLE)`
...
`r solend()` `r solbegin(label="2 - how many ppts?", slabel=F,show=T, toggle=params$TOGGLE)` For a quick "how many?", functions like `n_distinct()` can be handy:
...
`r solend()` `r solbegin(label="3 - how many observations per ppt?", slabel=F,show=T, toggle=params$TOGGLE)` Here are the counts of trials for each participant.
...
`r solend()` `r solbegin(label="4 - how many observations for each audio type per ppt?", slabel=F,show=T, toggle=params$TOGGLE)` For this kind of thing I would typically default to using `table()` for smaller datasets, to see how many datapoints are in each combination of `PID` and `audio`:
...
# 5.2
print("audio and headphones")

# 5.3
print("Each row is a separate trial")

# 5.4
print("Yes, grouped by participants")

# 5.5
print("Different trials, type of audio")

# 5.6
print("Whether participants are wearing headphones or not")
```
...
`r qbegin(qcounter())` Make factors and set the reference levels of the `audio` and `headphones` variables to "no audio" and "speakers" respectively.
...
efdat = efdat %>% mutate(
  audio = factor(audio,
                 levels = c("no_audio", "music", "white_noise"),
                 ordered = TRUE),
  headphones = factor(headphones,
                      levels = c("S", "H"),
                      ordered = TRUE)
)
...
`r qbegin(qcounter())` Fit a multilevel model to address the aims of the study (copied below)
...
```{r}
sdmt_mod = lmer(SDMT ~ audio * headphones + (1 + audio | PID), efdat)
summary(sdmt_mod)
```

`r qend()` `r solbegin(label="1 - fixed effects", slabel=F,show=T, toggle=params$TOGGLE)`
...
`r solend()` `r solbegin(label="2 - hierarchical data structure", slabel=F,show=T, toggle=params$TOGGLE)`
...
`r solend()` `r solbegin(label="3 - random slopes", slabel=F,show=T, toggle=params$TOGGLE)`
...
```{r}
library(effects)
effects = effect(term = "audio*headphones", mod = sdmt_mod) |>
  as.data.frame()
effects %>%
  ggplot(aes(x = audio, y = fit, color = headphones)) +
  geom_point(size = 2)+
...
`r qbegin(qcounter())` Now we have some p-values and a plot, try to create a short write-up of the analysis and results.

```{r}
# Executive funtion are affected by different types of audio. Listening to music is associated with lower SDMT scores while listening to white noise is not significantly different from the no-audio condition. Participants with headphones are associated with higher SDMT scores in the no-audio condition. The interaction between headphones and audio-type was also significant in our model. The wearing headphones condition is associated with higher SDMT scores when listening to white noise compared to the speaker condition, but does not differ with the speaker condition when listening to music.
```
